...
def assign_tickets(assignedTicket : AssignTicketModel,backgroundTask : BackgroundTasks,current_user = Depends(is_authenticated), db = Depends(get_db)):
    return assignTicket(ticket_id=assignedTicket.ticket_id,technician_id=assignedTicket.technician_id, db=db, role=current_user.role, current_user= current_user, comment=assignedTicket.comment, backgroundTask = backgroundTask)
